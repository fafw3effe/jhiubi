        ...
        main.innerHTML = `
            <div class='container'>
                <div class='top-buttons'>
                    ...
                    <button class='top-btn' onclick='window.open("https://t.me/pptonindia", "_blank")'><i class='fab fa-telegram'></i> Telegram</button>
                </div>
    ...
    function renderPostPage(slug) {
        ...
        // Cast & genre
const toTags = (str, icon, limit) => str?.split(',').slice(0, limit).map(s => `${icon}<span class="meta-tag">${s.trim()} </span>`).join('') || '';
const genreHtml = toTags(movie.genre, '🎬');
const castHtml = toTags(movie.cast, '🎭', 3);
