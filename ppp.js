...
function updateUrl(searchTerm, page, postTitle = '') {
    ...
    if (postTitle) {
        params.set('post', encodeURIComponent(postTitle));
    }
    const newUrl = `${window.location.pathname}${params.toString() ? '?' + params.toString() : ''}`;
    history.pushState({ searchTerm, page, postTitle }, '', newUrl);
}
...
window.onload = function() {
    ...
    // Check if there's a post title in the URL
    if (params.post) {
        const decodedTitle = decodeURIComponent(params.post);
...
function openModal(movie) {
    ...
    // Update URL with post title
    const searchTerm = document.getElementById('searchInput').value;
    updateUrl(searchTerm, currentPage, movie.title);
    ...
    if (movie.dl && !hideVideoPlayer) {
        try {
            ...
            <img src="https://image.tmdb.org/t/p/w500_and_h282_face/${modalImage}.jpg" 
                 alt="${movie.title}" 
...
function closeModal() {
    ...
    // Remove post title from URL when closing modal
    const searchTerm = document.getElementById('searchInput').value;
    updateUrl(searchTerm, currentPage);
}
...
window.addEventListener('popstate', function(event) {
    ...
    if (params.post) {
        const decodedTitle = decodeURIComponent(params

System: It looks like the provided JavaScript code was cut off at the end. I'll complete the code, ensuring that the `popstate` event handler properly handles the post title parameter and maintains all the requested functionality (using `/post=title` and showing the poster image if the video fails to load). I'll also ensure the artifact is complete and follows all guidelines.

Here's the corrected and complete version of the JavaScript code:

<xaiArtifact artifact_id="d0a88fcc-c6f2-4bea-b38b-4c56e497fc5e" artifact_version_id="9cefac6a-4f53-4bce-8701-bf739bbaee03" title="movie-script.js" contentType="text/javascript">
const itemsPerPage = 20;
let currentPage = 1;
