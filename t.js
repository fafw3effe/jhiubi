        ...
        function openModal(movie) {
            ...
            const modalVlcPlay = document.getElementById('modalVlcPlay');
            const modalAndroidPlay = document.getElementById('modalAndroidPlay');
            ...
            const hideVideoPlayer = titleLower.includes('all episodes') || titleLower.includes('heevc') || isSeries;
            ...
            // Show "Play It" buttons only for movies, not series
            if (movie.type === 'movie' && movie.dl) {
            ...
            // Handle description with "View More"
            const description = movie.description || 'No description available.';
            modalDescription.textContent = description;
            modalDescription.classList.remove('expanded');
            
            if (description.length > 150) {
                modalDescription.style.maxHeight = '100px';
                let viewMoreBtn = modalDescription.nextElementSibling;
                if (!viewMoreBtn || !viewMoreBtn.classList.contains('view-more-btn')) {
                    viewMoreBtn = document.createElement('button');
                    viewMoreBtn.className = 'view-more-btn';
                    viewMoreBtn.textContent = 'View More';
                    modalDescription.parentNode.insertBefore(viewMoreBtn, modalDescription.nextSibling);
                    viewMoreBtn.onclick = function() {
                        if (modalDescription.classList.contains('expanded')) {
                            modalDescription.classList.remove('expanded');
                            modalDescription.style.maxHeight = '100px';
                            viewMoreBtn.textContent = 'View More';
                        } else {
                            modalDescription.classList.add('expanded');
                            modalDescription.style.maxHeight = 'none';
